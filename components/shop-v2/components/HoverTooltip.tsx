  ...
  const node = (
    ...
    >
      <div className="rounded-lg p-4 shadow-md" style={{ backgroundColor: tooltipBg, WebkitBackdropFilter: 'blur(4px)', backdropFilter: 'blur(4px)', color: fgColor }}>
        ...
        {((resolved?.atk !== undefined || resolved?.def !== undefined) && 
          ((resolved?.type && resolved.type.toLowerCase().includes('monster')) || 
           (resolved?.cardType && resolved.cardType.toLowerCase().includes('monster')))) && (
          <div className="mt-2 font-mono text-base" style={{ color: '#ffd86b' }}>
            <span>ATK/{resolved.atk === -1 ? '?' : (resolved.atk ?? 0)}</span>
            {(resolved.def !== undefined || resolved.def === 0) && (
              <span className="ml-3">DEF/{resolved.def === -1 ? '?' : (resolved.def ?? 0)}</span>
            )}
          </div>
        )}
        ...
        <div ref={tooltipScrollRef} className={`mt-2 text-sm text-white/90 max-h-48 overflow-y-auto leading-relaxed`}>
          {skills && skills.length > 0 ? (
            <CardDescription card={resolved} skills={skills} stats={hoverTooltip.stats} />
          ) : (
            <RichTextRenderer 
