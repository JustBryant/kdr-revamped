        ...
        <div className="flex items-center justify-between mt-12 pt-8 border-t border-gray-100 dark:border-white/5">
          <div className="flex-1">
            {onPurchase && !isBought && (
              <button
                type="button"
                onClick={(e) => { e.stopPropagation(); onPurchase(pool); }}
                className="p-4 px-10 rounded-xl bg-emerald-600 hover:bg-emerald-500 text-white font-black italic uppercase tracking-tighter text-xl shadow-xl shadow-emerald-500/10 transition-all active:scale-95 disabled:opacity-50"
                disabled={loading}
              >
                {loading ? 'Purchasing...' : `Buy for ${pool.cost}G`}
              </button>
            )}
          </div>
